...
class bsn_base_error(experimenter_error_msg):
    ...
    def unpack(reader):
        ...
        _experimenter = reader.read("!L")[0]
        assert(_experimenter == 6035143)
        obj.err_msg = reader.read("!256s")[0].rstrip(b"\x00")
        obj.data = reader.read_all()
        return obj
...
class bsn_image_desc_stats_reply(bsn_stats_reply):
    ...
    def unpack(reader):
        ...
        _subtype = reader.read("!L")[0]
        assert(_subtype == 14)
        obj.image_checksum = reader.read("!256s")[0].rstrip(b"\x00")
        obj.startup_config_checksum = reader.read("!256s")[0].rstrip(b"\x00")
        return obj
...
class bsn_lua_upload(bsn_header):
    ...
    def unpack(reader):
        ...
        assert(_subtype == 64)
        obj.flags = reader.read("!H")[0]
        obj.filename = reader.read("!64s")[0].rstrip(b"\x00")
        obj.data = reader.read_all()
        return obj
    ...
    def pretty_print(self, q):
        ...
        with q.group():
            with q.indent(2):
                ...
                q.text(","); q.breakable()
                q.text("reason = ");
                value_name_map = {0: 'OFP_BSN_CONTROLLER_ROLE_REASON_MASTER_REQUEST', 1: 'OFP_BSN_CONTROLLER_ROLE_REASON_CONFIG', 2: 'OFP_BSN_CONTROLLER_ROLE_REASON_EXPERIMENTER'}
...
class desc_stats_reply(stats_reply):
    ...
    def unpack(reader):
        ...
        obj.flags = reader.read("!H")[0]
        reader.skip(4)
        obj.mfr_desc = reader.read("!256s")[0].rstrip(b"\x00")
        obj.hw_desc = reader.read("!256s")[0].rstrip(b"\x00")
        obj.sw_desc = reader.read("!256s")[0].rstrip(b"\x00")
        obj.serial_num = reader.read("!32s")[0].rstrip(b"\x00")
        obj.dp_desc = reader.read("!256s")[0].rstrip(b"\x00")
        return obj
    ...
    def pretty_print(self, q):
        ...
        with q.group():
            with q.indent(2):
                ...
                q.text(","); q.breakable()
                q.text("code = ");
                value_name_map = {0: 'OFPTFFC_BAD_TABLE', 1: 'OFPTFFC_BAD_METADATA', 2: 'OFPTFFC_BAD_TYPE', 3: 'OFPTFFC_BAD_LEN', 4: 'OFPTFFC_BAD_ARGUMENT', 5: 'OFPTFFC_EPERM'}
                if self.code in value_name_map:
                    q.text("%s(%d)" % (value_name_map[self.code], self.code))
                else:
                    q.text("%#x" % self.code)
